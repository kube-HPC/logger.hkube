...
class Logger {
    constructor(containerName, config) {
        ...
        const { throttle, level } = this._options;

        const levels = this._createLevels(LEVELS);
        this._level = this._resolveLevel(level, levels);
    ...
    _loggerFormat({ level, message, meta }, format) {
        ...
        const currentFormat = format || this._defaultFormat();
    ...
    _defaultFormat() {
        return '{time} -> {level}: {component}{message} {extra}';
    }
    ...
    _createTransportFormat(config) {
        ...
        let formatData;
        if (formats.length) {
            formatData = winston.format.combine(...formats);
        }
        return { format: formatData, level, transportConfig };
    }
