...
class Logger {
    constructor(containerName, config) {
        ...
        const { format, throttle, verbosityLevel } = config.options || {};
        this._verbosityLevel = verbosityLevel;

        if (format) {
            this._printFormat = stringTemplate(format); // override current format
        }
        ...
        this.throttle = new Throttle(this, throttle);
    }
    ...
    _loggerFormat({ level, message, meta }) {
        ...
        if (this._config.options?.extraDetails) {
    ...
    static GetLogFromContainer(containerName) {
        ...
        if (!container.has(containerName)) {
            container.forEach((value,) => {
                if (value._config.options.isDefault) {
                    _logger = value;
                }
            });
        }
    ...
    trace(message, meta) {
        if (this._verbosityLevel <= levels.TRACE.level) {
            this._log(levels.TRACE.name, message, meta);
        }
    }

    silly(message, meta) {
        if (this._verbosityLevel <= levels.SILLY.level) {
            this._log(levels.SILLY.name, message, meta);
        }
    }

    debug(message, meta) {
        if (this._verbosityLevel <= levels.DEBUG.level) {
            this._log(levels.DEBUG.name, message, meta);
        }
    }

    info(message, meta) {
        if (this._verbosityLevel <= levels.INFO.level) {
            this._log(levels.INFO.name, message, meta);
        }
    }

    warning(message, meta) {
        if (this._verbosityLevel <= levels.WARN.level) {
            this._log(levels.WARN.name, message, meta);
        }
    }

    error(message, meta, error) {
        if (this._verbosityLevel <= levels.ERROR.level) {
            this._log(levels.ERROR.name, message, meta, error);
        }
    }

    critical(message, meta, error) {
        if (this._verbosityLevel <= levels.CRITICAL.level) {
