...
const os = require('os');
const logLevels = require('../consts/levels');
const { biggerLevelOperator, getLevelNumberByName } = require('../levelsOperations');
class RedisTransport extends winston.Transport {
	...
	_verbosityInterval() {
		...
		}, this.intervalTimedfasdsdfsf);
	}
	log(level, msg, meta, callback) {
		if (biggerLevelOperator(this.level, level)) {
			const output = this._log({
				colorize: this.colorize,
				json: this.json,
				level: level,
				message: msg,
				meta: { ...meta.meta, trace: undefined, serviceName: this.serviceName, podName: this.podName },
