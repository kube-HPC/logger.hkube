...
const { biggerLevelOperator } = require('../helpers/levelsOperations');
...
class RedisTransport extends winston.Transport {
    ...
    log({ level, message, meta }, callback) {
        if (biggerLevelOperator(level, this.level)) {
            const output = this._log({
                ...
                level,
                message,
                meta: { ...meta, serviceName: this.serviceName, podName: this.podName },
    ...
    _log(options) {
        let timestampFn = typeof options.timestamp === 'function' ? options.timestamp : this._timestamp,
